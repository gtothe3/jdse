...
dataDir <- "/Users/ggreivel/Desktop/ORwE/Nucor/Paper 3 Files-JSDSE/2024-07-01-Paper Revision Files/R Code and Data Files"
...
### User-Defined Standardized Residual Plots for Model N1

library(scales)
library(splines)

qqnorm(rstandard(naivelm), main = "Normal Q-Q Plot of Standardized Residuals (N1)", xlab = "Theoretical Quantiles", ylab = "Standardized Residuals")
qqline(rstandard(naivelm))

hist(rstandard(naivelm), main = "Histogram of Standardized Residuals (N1)", xlab = "Standardized Residuals", ylab = "Frequency")

fit_splineN1 <- smooth.spline(fitted(naivelm), rstandard(naivelm), spar = 1.1)
plot(fitted(naivelm), rstandard(naivelm), cex = 0.5, main = "Standardized Residuals vs. Fitted Values (N1)", xlab = "Fitted", ylab = "Standardized Residuals")
abline(h=0)
lines(fit_splineN1,lwd=2,col="red")
